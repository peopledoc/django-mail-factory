...
class MailFactory(object):
    ...
    def get_mail_class(self, template_name):
        ...
        if template_name not in self._registry:
            raise exceptions.MailFactoryError("%s is not registered" % template_name)
    ...
    def mail_admins(self, template_name, context, attachments=None, from_email=None):
        """Send a mail given its template name to admins."""
        mail = self.get_mail_object(template_name, context)
        mail.mail_admins(attachments, from_email)

    def get_html_for(self, template_name, context, lang=None, cid_to_data=False):
    ...
    def get_raw_content(
        self, template_name, emails, context, lang=None, from_email=None
    ):
        """Return raw mail source before sending."""
        mail = self.get_mail_object(template_name, context)
        return mail.create_email_msg(emails, from_email=from_email, lang=lang)
